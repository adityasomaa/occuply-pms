  ...
  return (
    <>
      ...
      <div className="flex flex-1 flex-col gap-4 p-3 lg:p-5">
        <StatStrip
          stats={[
            { label: "Active rate plans", value: String(activePlans.length), hint: `${ratePlans.length} configured in total` },
            { label: "ADR · 30d", value: moneyShort(kpi.adr), delta: kpi.adrDelta, hint: "Achieved, net of discounts", emphasis: true },
            {
              label: "Lowest sellable rate",
              value: moneyShort(Math.min(...roomTypes.map((r) => r.floorRate))),
              hint: "Rate floor across all room types",
            },
            {
              label: "Channels out of parity",
