  ...
  return (
    <>
      ...
      <div className="flex flex-1 flex-col gap-4 p-3 lg:p-5">
        <StatStrip
          stats={[
            { label: "Open tickets", value: String(open.length), hint: `${tickets.length - open.length} resolved this period` },
            {
              label: "Critical",
              value: String(critical.length),
              hint: critical.length ? critical[0].title : "Nothing critical outstanding",
              emphasis: critical.length > 0,
            },
            {
              label: "Rooms blocked",
            ...
            { label: "Estimated spend", value: moneyShort(spend), hint: "Across all open tickets" },
          ]}
        />
