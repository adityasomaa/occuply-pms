...
import { CheckIcon, ClockIcon, GlobeIcon, MailIcon, MapPinIcon, PhoneIcon, PlusIcon } from "lucide-react"
  ...
  return (
    <>
      ...
      <div className="flex flex-1 flex-col gap-4 p-3 lg:p-5">
        <StatStrip
          stats={[
            { label: "Properties", value: String(PROPERTIES.length), hint: "All in Bali, Indonesia" },
            { label: "Total units", value: String(portfolioUnits), hint: "Across every property" },
            {
              label: "Portfolio occupancy · 30d",
              value: percent(weightedOcc),
              hint: "Weighted by unit count",
              emphasis: true,
            },
            { label: "Portfolio revenue · 30d", value: moneyShort(portfolioRevenue), hint: "Room revenue only" },
          ]}
        />
